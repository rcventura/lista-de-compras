...
class CategoriesItemsBloc
  ...
  Future<void> _onLoadCategoriesItems(
    ...
    try {
      final categoriesItems = await _categoriesItemsUsecase
          .fetchCategoriesItems(event.categoryId);
      emit(CategoriesItemsLoadingSuccess(categoriesItems));
    } catch (e) {
      emit(
        CategoriesItemsLoadingError(
          'Erro ao carregar os produtos dessa categoria, tente novamente',
        ),
      );
    }
  }
